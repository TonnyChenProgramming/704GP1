...
Highlights
- Improved 3CD supports a true COMBINED / ALL3 option.
- COMBINED invokes the runner once with --targets ALL3.
- ALL3 means Batch + Production + Safety are stalled simultaneously inside
  one mission and share one outage window.
- With fault-reps=20, COMBINED therefore runs 20 missions, not 60.
- Individual BATCH / PRODUCTION / SAFETY results remain separate table rows.
...
class AvailabilityGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.run_all_btn = ttk.Button(actions, text="Run full comparison", command=self.run_all)
        ...
        note = (
            ...
            "Improved COMBINED / ALL3 is one simultaneous three-CD outage per mission; individual targets remain separate rows."
        )
    ...
    def _update_comparison(self, mode, target, bundle):
        ...
        target_key = target
        metrics_key = "ALL3" if target == COMBINED_LABEL else target
        metrics = bundle["targets"].get(metrics_key, bundle["all"])
        values = self._comparison_values(mode_label, target_key, metrics)
        self._upsert_row(f"{mode_slug}:{metrics_key}", values)
