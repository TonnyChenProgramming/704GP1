"""
Availability Experiment GUI (updated)

Highlights
- Improved 3CD now supports a COMBINED run option.
- When COMBINED is selected, the runner is invoked once with
  --targets BATCH,PRODUCTION,SAFETY.
- The session table shows four lines:
    Improved | BATCH
    Improved | PRODUCTION
    Improved | SAFETY
    Improved | COMBINED
- The live panel shows aggregate COMBINED metrics while the combined run is in
  progress, and still tracks the current sub-run from runner stdout.
...
COMBINED_RUNNER_TARGETS = "BATCH,PRODUCTION,SAFETY"
...
def read_live_breakdown(raw_csv: Path):
    bundle = {
        ...
        "baseline_drained": 0,
        "all": new_metrics(),
        "targets": {t: new_metrics() for t in ("COORDINATOR", "BATCH", "PRODUCTION", "SAFETY")},
    }
...
class AvailabilityGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.run_all_btn = ttk.Button(actions, text="Run all 3", command=self.run_all)
        ...
        note = (
            ...
            "Improved COMBINED shows both the per-target rows and the aggregate row."
        )
    ...
    def _runner_target(self, mode, target):
        if mode == "Improved 3CD" and target == COMBINED_LABEL:
            return COMBINED_RUNNER_TARGETS
        return target
    ...
    def run_all(self):
        ...
        self._start_mode("Baseline 1CD", "COORDINATOR")
    ...
    def _update_comparison(self, mode, target, bundle):
        ...
        if mode == "Improved 3CD" and target == COMBINED_LABEL:
            for t in ("BATCH", "PRODUCTION", "SAFETY"):
                values = self._comparison_values(mode_label, t, bundle["targets"][t])
                self._upsert_row(f"{mode_slug}:{t}", values)
            values = self._comparison_values(mode_label, COMBINED_LABEL, bundle["all"])
            self._upsert_row(f"{mode_slug}:{COMBINED_LABEL}", values)
            return
